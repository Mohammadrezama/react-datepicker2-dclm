...
import { englishNumber, persianNumber } from '../utils/persian';
...
// List of months
let yearsJalaali = range(momentJalaali(new Date()).jYear() + 100, 1300);
yearsJalaali = yearsJalaali.map(item => persianNumber(item));
...
export default class YearSelector extends Component {
  ...
  handleClick(key) {
    ...
    setMonth(momentJalaali(englishNumber(key), monthYearFormat));
      ...
      <div className="month-selector">
        ...
        <div ref={this.yearsContainerRef} className={styles.yearsList}>
          {years.map((yearItem, key) => {
            const buttonFingerprint = `${englishNumber(month.format(monthFormat))}-${years[key]}`;
